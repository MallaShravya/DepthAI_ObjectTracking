...
# Connecting to the tracklet database
connection = sqlite3.connect('trackletData.db')
...
videoPathDefault = str((Path(__file__).parent / Path('models/3_meeting_and_walking.mp4')).resolve().absolute())
...
with dai.Device(pipeline) as device:
    ...
    while cap.isOpened():
        ...
        for t in trackletsData:
            ...
            if t.status.name != 'LOST': #REMOVING THE 'LOST' TRACKLETS FROM THE DISPLAY
            ...
            if t.id not in thelistofids:
                thelistofids.append(t.id)
                ts_start = time.ctime()
                ts_end = time.ctime()
                # mainlist.append([t.id, ts_start, ts_end, t.status.name])

            else:
                # for i in range(len(mainlist)):
                #     if mainlist[i][0] == t.id:
                #         mainlist[i][2] = time.ctime()
                #         mainlist[i][3] = t.status.name
                ts_end = time.ctime()

            cursor.execute("INSERT INTO ids VALUES(?, ?, ?, ?)", (t.id, ts_start, ts_end, t.status.name))
...
# file = open('listofids.csv', 'w')
# writer = csv.writer(file)
# writer.writerow(['ID', 'STARTTIME', 'ENDTIME', 'STATUSATTHEEND'])
# for data_list in mainlist:
#     writer.writerow(data_list)
#
# file.close()

# Opening the person-records.csv file
# file1 = open('listofids.csv')
...
# person table
# insert_records = "INSERT INTO ids (id, starttime) VALUES(?, ?)"
